...
- **待办（明天）**：
  - 字体中文名显示为英文（霞鹜文楷→LXGW WenKai）：`readFamilyName` 已改为「优先含中文字符的记录」+ 字节序用健康 CJK 判据，**代码已改，未跑通单测**（上次 `Source Han Serif` 被判成乱码，已改判据，需重跑 `FontParserTest` 并真机验证）。
...
- [ ] **② 主题选择列表 UI**：设置面板「排版主题」列表单选（现代/传统/原书/…），切换即注入对应主题 css 实时生效 + 持久化。
- [ ] **③ 界面控件常驻层**：字号滑块(根 em)/行距/边距，跨主题保持；日夜整套配色切换 + 单独覆盖背景色。
