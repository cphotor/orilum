...
- **封面全屏显示（实施中，待合并）**：正文首屏封面统一「等比缩放留白边 / 拉伸铺满整屏」开关（`coverProportional`，默认等比留白）；统一 img/svg 封面渲染（svg 以 `<image>` 完整尺寸改写 `viewBox` 防裁切/放大超屏）；开场动画封面跟随同一开关，且延迟到 settingS 就绪后一次以正确形态显示（消除「先白边再跳全屏」）；开关持久化与其它排版字段同套分层（每书私有覆盖 + 全局传染新书）。
