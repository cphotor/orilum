...
## 近期更新 · 分页模型重构（屏级小节 + 字符偏移锚点）

> 大章节（如 GIMP 3.0 用户手册）以「章」为排版单位导致各种操作全卡、改设置基点漂移。方案已确定（详见 [`ARCHITECTURE.md`](ARCHITECTURE.md) §8），**待落地**。

- **方案定案**：**① 不自写分页内核**（LaTeX/MathML/SVG/表格跨页断行依赖浏览器），**② 只渲染少量页**——把大章拆成 N 个「屏级小节」（每节≈1~2 屏）塞入 `book.sections`，foliate 三窗口只渲染 3 节；**③ 进度/书签/基点用「字符偏移锚点」**而非像素 page index，重排后锚回原段文字，改字号/行距/旋转屏基点不漂移。
- **当前阶段**：设计已书面化，进入落地前的改造规划。
