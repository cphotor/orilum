...
- ~~**封面全屏显示**~~ **✅ 已完成（v0.3.2）**：正文首屏封面统一「等比缩放留白边 / 拉伸铺满整屏」开关（`coverProportional`，默认等比留白）；统一 img/svg 封面渲染（svg 以 `<image>` 完整尺寸改写 `viewBox` 防裁切/放大超屏）；开场动画封面跟随同一开关，且延迟到 settings 就绪后一次以正确形态显示（消除「先白边再跳全屏」）；开关持久化与其它排版字段同套分层（每书私有覆盖 + 全局传染新书）。两种首页情形统一处理：Spine[0] 已是封面大图 → 走 `isCoverLike` 全屏去边距；Spine[0] 非封面但 metadata 声明封面 → 合成全屏封面章（`<svg><image>` + 同源 blob URL），在 `view.open` 前插入 spine 首位 index 0，保证目录/进度/CFI 索引自封面起一致。
- ~~**书架双视图 / 编辑 / 排序**~~ **✅ 已完成（v0.3.3）**：封面/列表双视图切换（封面网格 平板4列/手机2列、带图列表）；编辑态多选删除（长按进入、点选切换、全选/反选、删除即清理文件与封面）；排序（加入时间/阅读时间/书名）；视图与排序持久化；导入去重（书名+作者判重，一次性弹窗询问覆盖/跳过）。
...
- `v0.3.2`：封面全屏显示——正文首屏封面统一 `coverProportional` 开关（等比/拉伸）；Spine[0] 封面大图走 `isCoverLike` 全屏，Spine[0] 非封面但 metadata 声明封面则合成全屏封面章插入 spine 首位；开场动画封面跟随同一开关。
- `v0.3.3`：书架功能批次——封面/列表双视图切换、编辑态多选删除（长按进入/点选切换/全选/反选/清理文件与封面）、排序（加入时间/阅读时间/书名）、视图与排序持久化、导入去重覆盖确认弹窗。
- （后续每次合并补齐，如 `v0.3.4`…）
