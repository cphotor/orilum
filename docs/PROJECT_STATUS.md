...
- [x] **③ 界面控件常驻层 ✅（已实现 + 单测）**：字号滑块(根 em)/行距/边距跨主题保持（即便切「原书设置」字号行距仍可调，属「随时可调」最高层）；日夜**整套配色**切换 + **单独覆盖背景色**。
  - **字号（根 em · 相对默认）**：UI 归一为 0–100 滑块，**分段映射** `0..50 → 缩放 0.5..1.0`、`50..100 → 1.0..2.0`，**50 档 = 各元素自身默认**（根锚=正文 18px，h1/code 等随其 em 基准等比缩放，不丢默认基准）。注：设计文档示例公式 `em=0.5+v/100×1.5`（50→1.25）与「50=默认」矛盾，经确认采用**分段映射**。旧套绝对 `fontSize(px)` 自动反解迁移为档位。
  - **日夜整套配色**：`SCHEMES`(day/night) 定义「背景 + 正文字色」成套，`buildReadingCSS` 同时注入 `html`/`body` 的 `background+color`（根治「只切背景不切文字 → 夜黑字黑」）。
  - **单独覆盖背景色**：`bgOverride`（hex，空=跟随时）经 swatch 色板 + 原生取色器设置，属独立覆盖层；日夜切换按整套配色走。
  - **界面常驻层换肤**：`body[data-scheme]` 驱动 TOC/设置抽屉/字体选择/各类控件 CSS 变量整体换肤（面板 bg/文字/边框/选中高亮/开关/滑块等）。
  - **☾夜间按钮**：底部工具行「夜间」接入，一键日夜整套切换（原为占位）。
  - **设置持久化补齐**：`ReaderSettings` 新增 `fontScale/scheme/bgOverride/layoutTheme` 并纳入 `toJson/fromJson` —— 修掉此前「JS 里 `layoutTheme`(排版主题) 等字段走 Kotlin 白名单数据类被丢弃、跨重启不持久」的隐患；`fromJson` 同时做旧套 `theme→scheme/bgOverride`、`fontSize→fontScale` 迁移。`ReaderSettingsTest` 11 用例全绿（含映射/迁移/round-trip）。
