...
> 以 **foliate-js 渲染 + 自建 Kotlin 数据层** 为核心，取代早期基于 `EpubNavigatorFragment` 的方案（后者分页锁死双栏对开、且无连续滚动与卷页能力）。
...
- **UI 归一为 0–100 滑块，分段映射**：`0..50 → 缩放 0.5..1.0`、`50..100 → 1.0..2.0`，**50 档 = 各元素自身默认**（根锚=正文 18px，h1/code 等随其 em 基准等比缩放）。相对默认语义：滑块不丢默认基准；不用“绝对区间”语义（那会让 code 默认被压到 1.0）。
